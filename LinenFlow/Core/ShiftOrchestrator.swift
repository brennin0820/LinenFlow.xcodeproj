...
@MainActor
@Observable
final class ShiftOrchestrator {
    enum ReconciliationTrigger: Sendable, CustomStringConvertible {
        case appForeground
    ...
    func acknowledge(phase: ShiftTimelinePhase) async {
        ackState.acknowledge(phase)
        await reconcile(trigger: .notificationAction(phase, acknowledged: true))
    }

    func snooze(phase: ShiftTimelinePhase) async {
