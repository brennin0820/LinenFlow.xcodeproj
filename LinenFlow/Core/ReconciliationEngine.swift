...
struct ReconciliationEngine: Sendable {
    ...
    func shouldStartLiveActivity(phase: ShiftTimelinePhase) -> Bool {
        phase >= .wake && phase <= .shiftActive
    }

    func shouldEndLiveActivity(timeline: ShiftTimelineSnapshot, now: Date) -> Bool {
    ...
    struct MonitoredRegion: Sendable {
    ...
    @discardableResult
    mutating func applyGeofenceEvent(
    ...
    func shouldUseSignificantChangeMonitoring(
        tier: ShiftPlannerSettings.MonitoringTier,
        locationState: ShiftLocationState
    ) -> Bool {
        tier == .activeCommute
