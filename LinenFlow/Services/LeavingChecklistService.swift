...
enum LeavingChecklistService {
    static func buildDefaultItems() -> [LeavingChecklistItem] {
        [
    ...
    static func activeItems(from items: [LeavingChecklistItem]) -> [LeavingChecklistItem] {
        items.filter(\.isEnabled).sorted { $0.sortOrder < $1.sortOrder }
    }

    static func buildNotificationBody(from items: [LeavingChecklistItem]) -> String {
    ...
    static func calculateChecklistReminderTime(walkToCarTime: Date, remindBeforeMinutes: Int) -> Date {
        walkToCarTime.addingTimeInterval(-Double(remindBeforeMinutes) * 60)
    }

    static func shouldResetCheckedItems(lastResetDate: Date?, referenceDate: Date = .now) -> Bool {
