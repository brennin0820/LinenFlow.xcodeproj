...
enum TowerConfigService {
    static let minimumFloorCount = 1
    static let maximumFloorCount = 80

    enum FloorCountUpdateResult: Equatable {
        case updated(newCount: Int)
        case clampedToPolicy(protectedCount: Int)
        case unchanged
    }

    enum FloorRangeUpdateResult: Equatable {
        case updated(floorCount: Int, floors: [Int])
        case invalidRange
    }

    /// Effective delivery floor count, honoring operational policy when set.
    static func effectiveFloorCount(for tower: Tower) -> Int {
