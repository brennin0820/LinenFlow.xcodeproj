...
public struct ElevatorTripPlan: Hashable, Sendable {
    public var trips: [ElevatorTrip]
    public var completedTripIDs: Set<UUID>

    public var totalTrips: Int { trips.count }
    public var remainingTrips: Int { trips.filter { !completedTripIDs.contains($0.id) }.count }
    public var nextTrip: ElevatorTrip? { trips.first { !completedTripIDs.contains($0.id) } }
...
public struct ElevatorTripPlanner: Sendable {
    public func plan(
    ...
    private struct Load: Hashable {
        ...
        public var baseItemName: String
