...
public enum WorkdayPlannerService {
    // 1=Sun, 2=Mon, 3=Tue, 4=Wed, 5=Thu, 6=Fri, 7=Sat
    public static let defaultWorkdays: Set<Int> = [1, 2, 5, 6, 7]

    public static func buildDefaultSchedule() -> [WorkScheduleDay] {
        (1...7).map { weekday in
            WorkScheduleDay(
                weekday: weekday,
    ...
    public static func calculateStartDrivingTime(targetArrivalTime: Date, driveMinutes: Int) -> Date {
        targetArrivalTime.addingTimeInterval(-Double(driveMinutes) * 60)
    }

    public static func calculateWalkToCarTime(startDrivingTime: Date, walkToCarMinutes: Int) -> Date {
        startDrivingTime.addingTimeInterval(-Double(walkToCarMinutes) * 60)
    }

    public static func calculateBufferStartTime(walkToCarTime: Date, safetyBufferMinutes: Int) -> Date {
        walkToCarTime.addingTimeInterval(-Double(safetyBufferMinutes) * 60)
    }

    public static func calculateStartGettingReadyTime(bufferStartTime: Date, prepMinutes: Int) -> Date {
        bufferStartTime.addingTimeInterval(-Double(prepMinutes) * 60)
    }

    public static func calculateLeaveSoonTime(walkToCarTime: Date, leaveSoonAlertMinutes: Int) -> Date {
        walkToCarTime.addingTimeInterval(-Double(leaveSoonAlertMinutes) * 60)
    }

    public static func calculateShiftSoonTime(shiftStartTime: Date, shiftSoonAlertMinutes: Int) -> Date {
        shiftStartTime.addingTimeInterval(-Double(shiftSoonAlertMinutes) * 60)
    }

    public static func calculateChecklistReminderTime(walkToCarTime: Date, checklistReminderBeforeMinutes: Int) -> Date {
        walkToCarTime.addingTimeInterval(-Double(checklistReminderBeforeMinutes) * 60)
    }

    public static func buildWorkdayPlan(
        weekday: Int,
        shiftStartDate: Date,
    ...
    public static func nextWorkShift(schedule: [WorkScheduleDay], from referenceDate: Date = .now) -> (WorkScheduleDay, Date)? {
        let calendar = Calendar.current
        for offset in 0...7 {
    ...
    public static func isWorkday(_ schedule: [WorkScheduleDay], date: Date = .now) -> Bool {
        let calendar = Calendar.current
        let weekday = calendar.component(.weekday, from: date)
        return schedule.first(where: { $0.weekday == weekday })?.isWorkday ?? false
    }

    public static func validateSchedule(_ schedule: [WorkScheduleDay]) -> [String] {
        var warnings: [String] = []
