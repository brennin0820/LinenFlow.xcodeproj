...
public enum ShiftPlannerPersistence {
    public static let appGroupID = SharedWidgetStateManager.appGroupID
    ...
    public static func loadLocationState() -> ShiftLocationState {
    ...
    public static func saveLocationState(_ state: ShiftLocationState) {
        guard let data = try? JSONEncoder().encode(state) else { return }
        defaults.set(data, forKey: locationStateKey)
        HimmerFlowLog.persistence.debug("Saved location state")
    }

    public static func loadAcknowledgementState() -> AcknowledgementState {
    ...
    public static func saveAcknowledgementState(_ state: AcknowledgementState) {
        guard let data = try? JSONEncoder().encode(state) else { return }
    ...
    public static func saveActivityID(_ id: String?) {
        if let id {
    ...
    public static func resetSessionFlags() {
