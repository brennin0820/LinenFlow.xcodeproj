...
public struct TowerDisplayGroupSection: Identifiable {
    public let group: TowerDisplayGroup
    public let towers: [Tower]

    public var id: TowerDisplayGroup { group }
}

public struct LinenItemDisplayGroupSection: Identifiable {
    public let group: LinenItemDisplayGroup
    public let items: [LinenItem]

    public var id: LinenItemDisplayGroup { group }
}

public struct TowerParRequirement: Identifiable, Hashable {
    public let id: UUID
    public let itemName: String
    public let floorCount: Int
    public let parPerFloor: Int
    public let bundleSize: Int
    public let requiredPieces: Int
    public let requiredBundles: Int
    public let receivedPieces: Int
    public let receivedFullBundles: Int
    public let summaryDifferencePieces: Int?
    public let summaryDifferenceBundles: Int?

    public var receivedBundles: Int {
        receivedFullBundles
    }

    public var pieceGap: Int {
        summaryDifferencePieces ?? (receivedPieces - requiredPieces)
    }

    public var bundleGap: Int {
        summaryDifferenceBundles ?? (receivedBundles - requiredBundles)
    }
}

public struct FloorDeliveryAmount: Identifiable, Hashable {
...
public final class FlowViewModel {
    ...
    public static let currentTripMaxItems = 2
    ...
    public func configureWidgetShiftSettings(_ shiftSettings: ShiftSettings) {
        widgetShiftSettings = shiftSettings
    }

    // MARK: - Widget item pinning

    public var pinnedWidgetItemNames: [String] {
        get {
    ...
    public func isWidgetPinned(_ itemName: String) -> Bool {
        pinnedWidgetItemNames.contains(itemName)
    }

    public func toggleWidgetPin(for itemName: String) {
    ...
    public func selectTower(_ tower: Tower) {
    ...
    public func updateSelectedTowerFloorCount(_ floorCount: Int) {
    ...
    public func selectedItemIDs(for tower: Tower) -> Set<UUID> {
        Set(availableItems.filter { itemIsAvailable($0, for: tower) }.map(\.id))
    }

    public func itemIsAvailable(_ item: LinenItem, for tower: Tower) -> Bool {
    ...
    public var towerDisplayGroups: [TowerDisplayGroupSection] {
    ...
    public func itemDisplayGroups(for tower: Tower?) -> [LinenItemDisplayGroupSection] {
    ...
    public var towerParRequirements: [TowerParRequirement] {
    ...
    public func saveSelectedItems(_ selectedItemIDs: Set<UUID>, for tower: Tower) throws {
    ...
    public func addOrUpdateReceivingEntry(
        item: LinenItem,
        binCount: Int? = nil,
    ...
    public func addOrUpdateReceivedPieces(item: LinenItem, pieces: Int) {
    ...
    public func addCartLabelRow(item: LinenItem, pieces: Int, cartNumber: String? = nil) {
    ...
    public func removeReceivingEntry(_ entry: ReceivingEntry) {
    ...
    public func randomizeReceivingEntriesForTesting() {
        guard let tower = selectedTower else { return }
    ...
    public func selectCurrentDeliveryItem(_ itemName: String?) {
        setCurrentDeliveryItem(itemName)
    }

    // MARK: - Delivery session

    public func startDeliverySession() {
        guard let tower = selectedTower else { return }
    ...
    public func pauseDeliverySession() {
    ...
    public func resumeDeliverySession() {
        guard deliverySessionState.isActive, deliverySessionState.isPaused else { return }
    ...
    public func finishDeliverySession() {
    ...
    public func refreshFloorSensing() {
    ...
    public func stopFloorSensing() {
        floorSensingService.stop()
        floorSensingState = nil
    }

    public func correctFloorSensing(to floor: Int) {
        guard let tower = selectedTower else { return }
        let floors = deliverySessionState.deliveryFloors
    ...
    public func markFloorComplete(_ floorNumber: Int) {
    ...
    public func unmarkFloorComplete(_ floorNumber: Int) {
        ensureDeliverySessionPrepared()
    ...
    public func toggleFloorComplete(_ floorNumber: Int) {
    ...
    public func setCurrentDeliveryItem(_ itemName: String?) {
    ...
    public var smartTipsEnabled: Bool { smartTipsService.smartTipsEnabled }
    public var autoOpenTipsEnabled: Bool { smartTipsService.autoOpenTipsEnabled }
    public var showTipButtons: Bool { smartTipsService.showTipButtons }

    public func setSmartTipsEnabled(_ enabled: Bool) {
        smartTipsService.smartTipsEnabled = enabled
        if !enabled { activeSmartTip = nil }
    }

    public func setAutoOpenTipsEnabled(_ enabled: Bool) {
        smartTipsService.autoOpenTipsEnabled = enabled
    }

    public func setShowTipButtons(_ enabled: Bool) {
        smartTipsService.showTipButtons = enabled
    }

    public func dismissSmartTip(markDismissed: Bool) {
    ...
    public func turnOffSmartTips() {
        activeSmartTip = nil
        smartTipsService.smartTipsEnabled = false
    }

    public func presentSmartTip(_ tipID: SmartTipID, force: Bool = false) {
    ...
    public func autoPresentSmartTip(_ tipID: SmartTipID) {
    ...
    public func resetDismissedSmartTips() {
    ...
    public var deliveryFloorNumbersForCurrentTower: [Int] {
        deliverySessionState.deliveryFloors
    }
    public var remainingDeliveryFloorCount: Int {
        deliverySessionState.remainingCount
    }
    public var completedDeliveryFloorCount: Int {
        deliverySessionState.completedCount
    }
    public var currentTripItemNames: [String] {
        currentTripItemNamesSet
    }

    // MARK: - Current Trip (carry plan)

    public var availableCurrentTripItems: [CalculationSummary] {
    ...
    public var currentTripRangesByItem: [FloorRangeGroup] {
    ...
    public func toggleCurrentTripItem(_ itemName: String) {
    ...
    public func clearCurrentTripItems() {
    ...
    public func updateTowerFloorRange(_ tower: Tower, startFloor: Int, topFloor: Int, skip13thFloor: Bool) {
    ...
    public func eraseAllDataAndReset(isCustomProperty: Bool) {
    ...
    public func resetTowerFloorRangeToDefaults(_ tower: Tower) {
    ...
    public func deleteTower(_ tower: Tower) {
        if selectedTower?.id == tower.id {
            if deliverySessionState.isActive, deliverySessionState.towerName == tower.name {
                resetDeliverySessionAndEndActivity()
            }
    ...
    public func setTowerActive(_ tower: Tower, isActive: Bool) {
    ...
    public var nextUndoneDeliveryFloor: Int? {
        deliverySessionState.deliveryFloors.first(where: { !deliverySessionState.completedFloorNumbers.contains($0) })
    }

    public func nextUndoneFloor() -> Int? {
        nextUndoneDeliveryFloor
    }

    public func itemsNeededOnFloor(_ floorNumber: Int) -> [String] {
        var seen = Set<String>()
        var result: [String] = []
    ...
    public func deliveryAmounts(onFloor floorNumber: Int) -> [FloorDeliveryAmount] {
    ...
    public func markFloorCompleteAndAdvance(_ floorNumber: Int) {
        ensureDeliverySessionPrepared()
        guard deliverySessionState.deliveryFloors.contains(floorNumber) else { return }
    ...
    public func processPendingLiveActivityDrops() {
    ...
    public func setNextCarryGroup(_ title: String?) {
    ...
    public var usesPieceDelivery: Bool {
        guard let tower = selectedTower else { return false }
        return tower.deliveryMode == .pieces
    }

    public var prefersBundleDelivery: Bool { !usesPieceDelivery }

    public var deliveryFloorDistributions: [FloorDistributionRow] {
        usesPieceDelivery ? floorDistributions : bundleFloorDistributions
    }

    public var deliveryUnitIsBundles: Bool { !usesPieceDelivery }

    public var usesParSystem: Bool {
        TowerOperationalPolicy.usesParSystem(for: selectedTower)
    }

    public var carryGroups: [CarryGroup] {
    ...
    public func updateShiftIntelligence(from logs: [DailyLog]) {
        guard let tower = selectedTower else {
            supplyPredictions = []
            supplyAnomalies = []
            return
    ...
    public var smartFillItemCount: Int {
        supplyPredictions.filter(\.hasValue).count
    }

    public var smartFillSummary: String? {
    ...
    @discardableResult
    public func applySmartFill() -> Int {
    ...
    public func loadFromLog(_ log: DailyLog) {
    ...
    public func loadDiamondDHeadExample() {
        resetFlow()
        refreshAvailable()
        isDemoDay = true
    ...
    public func buildDailyLog() -> DailyLog? {
